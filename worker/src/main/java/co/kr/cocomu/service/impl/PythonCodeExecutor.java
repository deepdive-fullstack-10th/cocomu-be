...
import co.kr.cocomu.docker.container.PythonContainer;
...
public class PythonCodeExecutor implements CodeExecutor {
    ...
    private final static String FILE_NAME = "Main.py";

    @Override
    public EventMessage<ExecutionMessage> execute(final CodeExecutionMessage message) {
        final String containerId = "python-exec-" + UUID.randomUUID();
        Path tempDir = null;

        try {
            tempDir = TempDirectoryManager.createTempDirectory();
            CodeFileManager.createFile(tempDir, FILE_NAME, message.code());
            final List<String> dockerCommand = PythonContainer.generate(containerId, tempDir, message.input().trim());
